...
# cluster version
# get the list of supported versions with `az aks get-versions --location eastus`
AZ_CLUSTER_VERSION=${CLUSTER_VERSION:-"1.18.2"}
...
create)
  ...
  $EXE_AZ aks create \
    ...
    --kubernetes-version "$AZ_CLUSTER_VERSION" \
    --generate-ssh-keys \
    $AZ_CREATE_ARGS || {
